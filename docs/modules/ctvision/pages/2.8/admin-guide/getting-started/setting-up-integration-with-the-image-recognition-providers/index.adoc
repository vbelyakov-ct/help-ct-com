= 5. Setting up Integration with the Image Recognition Providers
...
[NOTE]
====
For each xref:2.8/ref-guide/vision-settings-ref/vision-visit-field-reference.adoc[Vision Visit] record type, the [.apiobject]#Provider__c# field must be filled with the corresponding value for the image recognition provider.
====

The list of available providers in the  *Recognition Engine*  picklist:

* CG Cloud Einstein.  This provider is used in case of integration with xref:ctmobile:ios/mobile-application/mobile-application-modules/cg-cloud/index.adoc[the CG Cloud solution].
* CT Vision
* EasyPicky
* Einstein
* Intelligence Retail
* Planorama
* xref:ctvision-lite:index.adoc[Vision Light]. Select this provider when image recognition is not required.
