...
learn more:
** xref:android/quick-reference-guides/installing-ct-mobile-package/ct-mobile-managed-package-update-to-v-3-54.adoc[CT Mobile
...
. Please set up xref:android/knowledge-base/configuration-guide/remote-site-settings.adoc[the required remote
sites].
. Assign xref:android/managing-ct-mobile-licenses[CT Mobile license] to
users.
. Specify xref:android/quick-reference-guides/application-permission-settings.adoc[permissions to
users]:
.. check that the user's profile has permission to read or edit
necessary objects and fields.
.. xref:android/quick-reference-guides/application-permission-settings.adoc#h3_2115044027[assign
required permission sets] to users according to their roles.
. ​Ensure that all required objects
xref:android/managing-offline-objects#h2_1551357854[are available offline]
and xref:android/managing-offline-objects#h2_1814060569[use SOQL filters]
to reduce the synchronization time and the amount of data to download.
. xref:android/installing-the-ct-mobile-app[Download the CT Mobile app].
. xref:android/logging-in[Log in] to the CT Mobile app using your
Salesforce credentials.
. Perform xref:android/quick-reference-guides/synchronization-launch/index.adoc[the first full
synchronization].
