...
xref:using-calendar#h3_860090196[month view].
...
xref:attaching-files-in-the-files-section#h3_1261438429[attached
...
the xref:ctm-settings-offline-objects[Offline objects] record
type.
...
* The xref:ctm-settings-offline-objects[Files Download Enabled]
...
will be reduced.
* The xref:ctm-settings-offline-objects[Files Filter] field
added for Offline objects. You can
xref:attaching-files-in-the-files-section#h3_1720123861[specify the
...
xref:attaching-files-in-the-files-section[Files] section of an
offline object. You can add or delete files in the
xref:attaching-files-in-the-files-section#h3_2018099316[Actions]
menu of an offline object.
* Adding and pinning list views for the
xref:using-calendar#h3_632854949[Calendar].
...
xref:offline-supported-operators-and-functions-in-field-with-the-formula-type[formulas].
...
* The ability to minimize the xref:libraries[Libraries] content and
...
* xref:ct-mobile-control-panel-calendar#h3_808313222[The default
...
out xref:reference-fields#h2_1554731138[the reference field].
...
* To go to the record screen, xref:using-calendar#h3_243561528[use
...
* xref:routes#h3_1243841833[The route points] are now numbered, and
...
xref:list-views#h2_380480215[list views].
...
xref:start-finish-functionality#h2_239706372[the unfinished
...
xref:chatter#h2_779708391[Chatter] posts and comments.
...
xref:using-calendar[in the calendar grid].
...
xref:applications[offline], used in the
...
screen.
* xref:search[Searching in the left record list] now displays the
...
xref:using-calendar[the Calendar module], the agenda with records
...
* In the xref:routes[Routes] module, the selected options are
saved.
...
xref:gestures-in-clm-presentations[a two-finger pinch].
...
* Refreshed xref:sync-recovery[Sync Recovery] functionality allows
...
in xref:clm-navigation-in-clm-presentations[the href attribute] of
the slide layout.
...
in xref:nearby-accounts[Nearby Accounts] and
xref:routes[Routes] modules, in xref:ctmobile:main/admin-guide/mobile-layouts/index.adoc-maps[the
...
filter records in xref:applications[the Applications module].
...
** New icons, xref:application-theme[vivid app themes], and modern
fonts.
** Upgraded xref:calendar[Calendar], xref:routes[Routes], and
xref:nearby-accounts[Nearby Accounts] modules with user-friendly
logic.
...
views on your iPad.
** xref:clm-presentation-controls[Multitasking during an activity]
...
* xref:libraries[The content module] now supports folder hierarchy.
* xref:sync-recovery[Sync Recovery]. Recovering the records which
could not be properly sent to Salesforce due to permissions issues.
* Adjust the xref:columns-width-for-related-lists[width of columns]
of the related lists and custom related lists.
* The long-hoped-for plus button is now available in the *Calendar*. It
is another way xref:using-calendar[to create an activity in the
...
* xref:set-up-holidays[Salesforce Holidays] are now supported and
are highlighted in the app's calendar.
* xref:ct-mobile-control-panel-general[Image Quality]. Set the
...
* xref:libraries[The Libraries module] download process can be
...
[[h4_1292798904]]
===== xref:using-calendar.html[Calendar]
...
* xref:nearby-accounts[The Nearby Accounts module]. The records
...
* The xref:search[Search] box value lower limit is reduced from
...
* xref:clm-presentation-controls[Presentation exit screen
improvements].
* The xref:managing-offline-objects[offline-objects] customization
...
* The improved logic of xref:using-calendar[copying activities] in
the calendar.
...
new posts in xref:chatter[Chatter].
...
* Updated for iOS 11.
* xref:ical-synchronization[Two-way sync with iPad/iPhone internal
...
* Customizable set of xref:search[search fields] for each object.
* xref:reference-fields#h2_1554731138[Quick record creation in
...
* The xref:chatter[Chatter] functionality for iPhone was added.

[[h3_118294372]]
==== CLM-presentations

* JS Bridge methods added:
** xref:ctm-disableslidescroll[ctm.isableSlideScroll]: optional
lock for slides scrolling using gestures.
** xref:ctm-enableslidescroll[ctm.enableSlideScroll]: slide
scrolling unlock option.
** xref:ctm-storefile[ctm.storeFile]: saving a file attachment as
an attachment to a record.
** xref:ctm-getfile[ctm.getFile] - getting the contents of an
...
xref:applications#h3_1236408094[custom scenarios] in the
