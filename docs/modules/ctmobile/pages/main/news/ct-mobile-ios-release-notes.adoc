...
object xref:compact-layout[compact layout].
...
xref:home-screen#h3_1292798904[calendar widget] for the Home
Screen.
** User can see
...
* Records counter in a xref:list-views[list view] is now displayed
if there are more than 500 records.
* Files sharing workflow has been upgraded, including
xref:attaching-files-in-the-files-section#h3__1261438429[attached
...
* xref:actions#h2_1173923582[Export .csv] item added to the
...
xref:attaching-files-in-the-files-section#h3__2018099316[Actions]
menu of an offline object.
* Adding and pinning list views for the
xref:using-calendar#h3__632854949[Calendar].
...
one.
**** Now you can xref:application-settings#h3__611076828[set up the
...
xref:technical-requirements-for-devices-and-network[requirements
...
nearby stores],
xref:managing-visits-to-retail-stores#h2__1880003381[build optimal
routes], and
...
xref:managing-visits-to-retail-stores#h2__1592083570[to generate
...
** xref:setting-up-task-definitions-and-action-plan-templates#h2__1302650526[Monitor
key processes] such as
xref:managing-visits-to-retail-stores#h3_1867622910[inventory
audits],
xref:managing-visits-to-retail-stores#h3__696266799[promotion
checks], and
xref:managing-visits-to-retail-stores#h3__85077301[in-store
surveys] with predefined templates.
** xref:managing-visits-to-retail-stores#h3__481270469[Using
...
* Gather information
xref:managing-visits-to-retail-stores#h3__582599340[using custom
metrics] and
xref:managing-visits-to-retail-stores#h3__1516408339[perform ad-hoc
tasks].
...
out xref:reference-fields#h2__1554731138[the reference field].
...
* To go to the record screen, xref:using-calendar#h3__243561528[use
...
* In the left list of records xref:home-screen#h3_1292798904[of the
...
xref:list-views#h2__380480215[list views].
...
characters] are supported in xref:mobile-layouts[mobile layouts],
standard and custom xref:list-views[list views], and
xref:chatter#h2__779708391[Chatter] posts and comments.
...
* Use xref:clm-application[new attributes of CLM presentation] to
...
* xref:app-menu[Assign the icon for the main tab] to help a user
...
xref:application-settings#h3__353973580[The option to choose]
...
limitations]. We highly recommend xref:ctm-settings[referring to
...
allows making sure that xref:synchronization[the synchronization]
is complete.
...
xref:application-prompts-for-requesting-permissions[pop-ups] with
functionalities that need the requested permission.
* Labels of menu items and modules in the menu, such as *Home Screen*,
*Routes*, or *Libraries*, xref:renaming-menu-items-and-modules[can
...
xref:routes[Routes] modules, in xref:mobile-layouts-maps[the
Map widget], and within xref:actions[the Determine Location button]
in the combo-menu.
* Use list views of xref:clm-application[the Application object] to
...
* The xref:logging-in[Login screen] is implemented using the
xref:oauth-2-0[OAuth 2.0] protocol and standard Salesforce UI.
...
Salesforce.
* xref:mini-layouts[Mini Layouts] can be assigned to a specific
user profile.
* Each user profile can have its xref:custom-color-settings[custom
...
xref:sync-logs[reports to analyze] the synchronization.
...
* xref:metadata-archive[Metadata downloading] optimization.
...
* xref:ct-mobile-replication[Improved replication]. Shared records
...
clock adjustment.
* The xref:person-accounts[Person Account] record type can be
created from a reference field.
* *Dashboards*. Graphs with summary formulas involved are displayed
correctly on xref:home-screen[Home Screen].
...
* xref:synchronization[Selective synchronization] in case of user
...
positioning xref:actions[on activity start].
...
* xref:sync-logs[Application log records] are more detailed.
* xref:person-accounts[Person Accounts] will now display in *Nearby
...
* Optimized xref:synchronization[synchronization] for organizations
with an extended data model and large data volumes.
* Customizable set of xref:search[search fields] for each object.
* xref:reference-fields#h2__1554731138[Quick record creation in
...
* The xref:actions[Determine Geolocation] functionality in the
...
* xref:home-screen[Customizable Home screen] for iPhone added.
