...
presentations by xref:clm-presentation-controls[the control
buttons], xref:gestures-in-clm-presentations[gestures], or
...
locally in the mobile app. For xref:clm-customscenario[custom
...
xref:clm-slide[Slide] object, it provides the possibility to change
...
configured as xref:clm-slide[Required] and cannot be excluded from
the custom scenario:
...
. Tap *Send Selected*.
. Select xref:email-templates[an email template] if available.
