...
xref:mobile-layouts-applications[mobile layout] and manage CLM
presentations by xref:clm-presentation-controls[the control
buttons], xref:gestures-in-clm-presentations[gestures], or
xref:js-bridge-api[JS methods and links] on the slides. For each
...
xref:full-synchronization[full synchronization], as it is stored
...
xref:clm-application[Application record type].
...
xref:synchronization-launch#h3__116633872[Launch fast
...
available xref:synchronization[at the first synchronization] or
