...
If enabled, xref:demonstration-mode[the demonstration mode] allows
...
xref:ical-synchronization[one-way or both way synchronized] ;with
...
If enabled, the current user will receive
xref:nearby-accounts-notifications[a notification of nearby
accounts].
...
Change xref:application-pin-code[the passcode]. The option is
visible when the passcode is enabled
in xref:ct-mobile-control-panel-general[CT Mobile Control Panel:
...
CT Mobile allows xref:send-application-data-dump[sending a dump]
