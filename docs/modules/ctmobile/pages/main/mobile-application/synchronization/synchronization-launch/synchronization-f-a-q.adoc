...
The sync process may include the xref:sync-recovery[Sync Recovery]
functionality.
...
For more information, read xref:sync-recovery[the Sync Recovery
...
xref:send-application-data-dump#SendApplicationDataDump-SpecifyDatatoSend[Sync
...
may incorrectly work, for example, xref:validation[offline
validation rules].

. Check xref:ct-mobile-control-panel-tools#h3_2011978[the cloud
token] (refer to xref:ct-mobile-control-panel-tools#h3_2011978[CT
...
* Also, check the xref:application-language[Application Language]
article.
...
Go to the xref:send-application-data-dump[Send Application Data
Dump] article.
...
xref:managing-offline-objects[customize the criteria] to download
...
xref:managing-offline-objects[SOQL query] in the error message.
...
inserted into xref:reference-fields[the reference field] when the
...
* Disable the xref:ctm-settings-offline-objects[Files Download
Enabled] option for offline objects. The files attached to offline
objects will be downloaded as previews.
* Set up the xref:ctm-settings-offline-objects[Files Filter] option
