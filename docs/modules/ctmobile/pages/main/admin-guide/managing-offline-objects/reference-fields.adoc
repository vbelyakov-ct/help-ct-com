...
* xref:reference-fields#h2__1514470758[download additional
record] if the reference field refers to the not downloaded record;
* xref:reference-fields#h2__1554731138[search value] or create a
new record to add;
* use xref:reference-fields#h2__321327715[lookup filters] to see
records that match required or optional criteria.

[[h2__1514470758]]
=== Online One Record Fetching

Along with xref:online-records-fetching[online records fetching], a
...
record, on xref:mobile-layouts[the mobile layout] of the linked
record.
...
using xref:mini-layouts[a mini layout].
...
field by using xref:mini-layouts[a mini layout].
* Tap the *Clear* button to clear the value from the reference field. If
a reference field is empty, the *Clear* button is not displayed.
...
rebuild xref:metadata-archive[a metadata
archive] using xref:metadata-checker[one of the available
