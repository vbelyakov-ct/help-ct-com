...
If enabled, link:demonstration-mode.html[the demonstration mode] allows
...
device gallery. The new photos in link:actions.html[the CT Mobile
...
link:ical-synchronization.html[one-way or both way synchronized] ;with
...
link:event-notifications.html[a notification of upcoming events]. The
...
If enabled, the current user will receive
link:nearby-accounts-notifications.html[a notification of nearby
accounts].
...
* Order module application is added to the link:app-menu.html[App menu].
* The
...
Change link:application-pin-code.html[the passcode]. The option is
visible when the passcode is enabled
in link:ct-mobile-control-panel-general.html[CT Mobile Control Panel:
General]/link:ct-mobile-control-panel-general-new.html#h3_643998525[CT
Mobile Control Panel 2.0: General]. 

[[h3__1321398704]]
==== Log out

Logging out link:log-out.html[to reset all application data].
...
CT Mobile allows link:send-application-data-dump.html[sending a dump]
...
CT Mobile allows link:logging-in.html[multiple user access]. Changing
...
link:logging-in.html[To change the current user], select the *User*
...
* *link:log-out.html[Log out and Erase]*. Tap to log out from the CT
Mobile app.
