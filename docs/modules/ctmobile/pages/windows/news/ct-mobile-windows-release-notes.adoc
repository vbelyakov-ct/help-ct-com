...
* The user could link:ios/application-settings#h3_1819839796[change the
current password or update the password] in case it was changed on the
Salesforce side.
* link:ios/send-application-data-dump[The automatically sending sync
logs] according to the specified debug level.
* Use link:ios/fast-synchronization[the mixed synchronization] instead
...
link:ios/synchronization-launch#SynchronizationModes-ResumingtheSynchronization[continue
...
* Use link:ios/applications#h3_1236408094[Mobile Scenario Editor] for
...
* Use link:ios/chatter[the Chatter menu item] with *Groups* and
*People* feeds.
* The ability to add the *Chatter* tab
link:ios/mobile-layouts-chatter-feed[on the mobile layout] to view the
record feed.
...
Go back to
link:ios/ct-mobile-windows-release-notes#ListOfWinReleases[the list of
CT Mobile releases].
...
* The ability to attach photos to link:ios/actions[the Account&#44;
Contact&#44; and Activity records].
* Now, when filling out link:ios/reference-fields#h2__1554731138[the
reference field], it is possible not only to choose the existing record
but also to create a new one.
* Customizable link:ios/search[set of search fields].
...
* Ability to filter records in link:ios/custom-related-lists[the custom
...
* For data protection, set link:ios/application-pin-code[an
...
link:ios/reference-fields#h2__1514470758[Tap the cloud icon] to
download the record. Tap the arrow to open the record screen.
* The ability to link:ios/send-application-data-dump[send database dump
...
the laptop.
* link:ios/clm-slide[Restrict a scroll gesture and use of arrow keys]
on slides such as a menu slide or a questionnaire slide. Use
link:ios/methods-for-clm-presentation-navigation[the href attribute or
the CTAPPgoToSlide() method on HTML5] to jump to the corresponding
slide.
* link:ios/crm-data-display[Display of CRM data] within CLM
...
the object and its fields link:ios/ctm-update#h2__471282633[to fill out
these fields with values from the CLM presentation] without invoking JS
Bridge.
* JS Bridge improvements:
** link:ios/ctm-getfile[ctm.getFile]: obtain a local path to the file
attachment of the[.object]#Attachment# or
[.object]#Content Version# objects by using their ID.
** link:ios/ctm-storefile[ctm.storeFile]: Tap/left-click the *Delete*
...
link:ios/nearby-accounts[the Nearby Accounts module].
...
* link:ios/links-list[The Links module] is supported. The in-app
...
link:ios/installing-the-ct-mobile-app[Installing CT Mobile Windows from
...
* The multi-user mode.
* The fast and full link:ios/synchronization[synchronization modes].
...
* Create and manage link:ios/custom-related-lists[custom related
lists].
...
* The ability to link:ios/custom-color-settings[set the color theme]
for link:ios/managing-offline-objects[records in the left menu],
activities in the link:ios/calendar[Calendar], and
...
* View link:ios/mobile-layouts-applications[the carousel of favorite
applications and custom scenarios] on the record’s details screen.
* View the location of an organization or contact
link:ios/mobile-layouts-maps[on a map fragment] on the record’s details
screen.
...
* link:ios/search[Search] the object’s records in list view results.
...
current or planned location in the link:ios/nearby-accounts[Nearby
Accounts] module.
...
link:ios/libraries[Libraries] module.
...
link:ios/dashboards[Dashboards] module.



For more information about the CT Mobile functionality, please refer to
link:ios/ct-mobile-os-comparison[CT Mobile OS Comparison].
