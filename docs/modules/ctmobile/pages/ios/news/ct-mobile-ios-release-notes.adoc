...
See the
link:compatible-versions-of-ct-mobile-and-other-packages[compatibility
...
link:calendar#h3_645629234[left menu], if they are set up in the
object link:compact-layout[compact layout].
** Up to 5 events can be displayed for one day in the
link:using-calendar#h3_860090196[month view].
** Customizable *All activities per day* hint added to the
link:home-screen#h3_1292798904[calendar widget] for the Home
Screen.
** User can see
...
* Records counter in a link:list-views[list view] is now displayed
if there are more than 500 records.
* Files sharing workflow has been upgraded, including
link:attaching-files-in-the-files-section#h3__1261438429[attached
files for offline objects], link:sharing-clm-presentations[CLM
...
* link:actions#h2_1173923582[Export .csv] item added to the
...
the link:ctm-settings-offline-objects[Offline objects] record
type.
...
* The link:ctm-settings-offline-objects[Files Download Enabled]
...
will be reduced.
* The link:ctm-settings-offline-objects[Files Filter] field
added for Offline objects. You can
link:attaching-files-in-the-files-section#h3_1720123861[specify the
...
link:attaching-files-in-the-files-section[Files] section of an
offline object. You can add or delete files in the
link:attaching-files-in-the-files-section#h3__2018099316[Actions]
menu of an offline object.
* Adding and pinning list views for the
link:using-calendar#h3__632854949[Calendar].
...
link:offline-supported-operators-and-functions-in-field-with-the-formula-type[formulas].
...
* The ability to minimize the link:libraries[Libraries] content and
link:attachments-and-files[Attachments] of a record. Move the
...
one.
**** Now you can link:application-settings#h3__611076828[set up the
...
**** Opening draft order.
**** Finalizing order.
*** link:limitations-for-the-ct-orders-module[Recommended load
...
link:technical-requirements-for-devices-and-network[requirements
for devices] and installing
link:ct-mobile-package-release-notes[the latest CT Mobile package
version].
...
* link:ct-mobile-control-panel-calendar#h3_808313222[The default
...
Go back to link:ct-mobile-ios-release-notes#ListOfReleases[the list
...
* link:activating-cg-cloud-in-salesforce[Activate CG Cloud in your
Salesforce org] to work with retail stores
link:activating-cg-cloud-in-the-ct-mobile-app[in the CT Mobile
app].
* link:managing-visits-to-retail-stores#h2_1751285824[Find the
nearby stores],
link:managing-visits-to-retail-stores#h2__1880003381[build optimal
routes], and
link:managing-visits-to-retail-stores#h2_535537278[schedule your
...
link:managing-visits-to-retail-stores#h2__1592083570[to generate
...
** link:setting-up-task-definitions-and-action-plan-templates#h2__1302650526[Monitor
key processes] such as
link:managing-visits-to-retail-stores#h3_1867622910[inventory
audits],
link:managing-visits-to-retail-stores#h3__696266799[promotion
checks], and
link:managing-visits-to-retail-stores#h3__85077301[in-store
surveys] with predefined templates.
** link:managing-visits-to-retail-stores#h3__481270469[Using
Einstein Detection and CT Vision], audit shelves, measure share and
shelf, manage stock and optimize product sales.
** link:managing-visits-to-retail-stores#h3_190353401[Using CT
Orders], create your orders and manage deliveries.
* Gather information
link:managing-visits-to-retail-stores#h3__582599340[using custom
metrics] and
link:managing-visits-to-retail-stores#h3__1516408339[perform ad-hoc
tasks].
...
link:clm-user[are saved] after each synchronization to optimize the
...
link:coloring-menu-items-and-modules[that object icon] in the CT
Mobile app.

image:color_menu_item_ios_en.png[]
* Use the detailed list view to see records in full screen when filling
out link:reference-fields#h2__1554731138[the reference field].
...
* To go to the record screen, link:using-calendar#h3__243561528[use
...
* In the left list of records link:home-screen#h3_1292798904[of the
...
* link:routes#h3_1243841833[The route points] are now numbered, and
...
later. We recommend installing
link:ct-mobile-package-release-notes#h2_62734607[the latest CT
...
link:list-views#h2__380480215[list views].
...
link:start-finish-functionality#h2_239706372[the unfinished
...
* When link:log-out[logging out] of the CT Mobile app, a
...
link:api-key[API Key] custom setting.
* https://help.salesforce.com/articleView?id=fields_using_rich_text_area.htm&type=5[Special
characters] are supported in link:mobile-layouts[mobile layouts],
standard and custom link:list-views[list views], and
link:chatter#h2__779708391[Chatter] posts and comments.
...
link:using-calendar[in the calendar grid].
...
* Use link:clm-application[new attributes of CLM presentation] to
choose whether the CLM presentation should be available
link:applications[offline], used in the
link:remote-detailing-launch[Remote Detailing] meeting, or both.
...
* link:app-menu[Assign the icon for the main tab] to help a user
quickly find out the object he is working with when viewing the record
screen.
* link:search[Searching in the left record list] now displays the
...
link:using-calendar[the Calendar module], the agenda with records
...
* In the link:routes[Routes] module, the selected options are
saved.
...
Remember to link:publishing-clm-presentations[re-publish these CLM
presentations] and perform the fast sync. ====

* The ability to zoom in or out a CLM presentation using
link:gestures-in-clm-presentations[a two-finger pinch].
...
link:remote-detailing-launch-the-ct-mobile-app[with one tap].
** Multitasking: switch between the meeting window and record details
link:remote-detailing-1-0-ui-for-presenter[with one tap]. Resume
...
link:application-settings#h3__353973580[The option to choose]
...
limitations]. We highly recommend link:ctm-settings[referring to
...
* Refreshed link:sync-recovery[Sync Recovery] functionality allows
...
link:synchronization-launch[the fast and full synchronization] to
...
allows making sure that link:synchronization[the synchronization]
is complete.
...
* For easy work with link:attachments-and-files[CLM presentation
...
in link:clm-navigation-in-clm-presentations[the href attribute] of
the slide layout.
* Converting a CLM presentation from
link:creating-clm-presentation-from-pdf[a PDF
file] or link:creating-clm-presentation-from-powerpoint[PowerPoint
...
link:application-prompts-for-requesting-permissions[pop-ups] with
functionalities that need the requested permission.
* Labels of menu items and modules in the menu, such as *Home Screen*,
*Routes*, or *Libraries*, link:renaming-menu-items-and-modules[can
be translated] using custom labels.
* On the link:ipad[detailed list view] screen, remove a record as
well as create a record if permissions are granted.
* The support of the standard *Shipping Address* and *Billing Address*
fields as link:mobile-application-setup[Account Geolocation Field]
in link:nearby-accounts[Nearby Accounts] and
link:routes[Routes] modules, in link:mobile-layouts-maps[the
Map widget], and within link:actions[the Determine Location button]
in the combo-menu.
* Use list views of link:clm-application[the Application object] to
filter records in link:applications[the Applications module].
* To quickly return to link:calendar[the Calendar] after moving on
...
** New icons, link:application-theme[vivid app themes], and modern
fonts.
** Upgraded link:calendar[Calendar], link:routes[Routes], and
link:nearby-accounts[Nearby Accounts] modules with user-friendly
logic.
** link:ipad[The collapsible main menu] provides a much wider
working area on your iPad.
** link:ipad[Detailed List Views]. Look through the detailed list
views on your iPad.
** link:clm-presentation-controls[Multitasking during an activity]
...
** New UI for link:iphone[iPhone devices].
* The link:logging-in[Login screen] is implemented using the
link:oauth-2-0[OAuth 2.0] protocol and standard Salesforce UI.
* link:libraries[The content module] now supports folder hierarchy.
* link:sync-recovery[Sync Recovery]. Recovering the records which
could not be properly sent to Salesforce due to permissions issues.
* Adjust the link:columns-width-for-related-lists[width of columns]
of the related lists and custom related lists.
* The long-hoped-for plus button is now available in the *Calendar*. It
is another way link:using-calendar[to create an activity in the
...
* link:set-up-holidays[Salesforce Holidays] are now supported and
are highlighted in the app's calendar.
* link:ct-mobile-control-panel-general[Image Quality]. Set the
amount of image compression to control the data volume sent to
Salesforce.
* link:mini-layouts[Mini Layouts] can be assigned to a specific
user profile.
* Each user profile can have its link:custom-color-settings[custom
...
All functionality is available link:ipad[by clicking the name of
...
records of the link:sync-log[Sync Log] object with an attachment
containing detailing information (log file). Use
link:sync-logs[reports to analyze] the synchronization.
* New synchronization algorithm with accelerated identification of the
created and updated records.
* link:libraries[The Libraries module] download process can be
moved to the background, which significantly improves the
synchronization time.
* link:metadata-archive[Metadata downloading] optimization.
...
* link:ct-mobile-replication[Improved replication]. Shared records
...
* link:nearby-accounts[The Nearby Accounts module]. The records
...
* The link:search[Search] box value lower limit is reduced from
...
clock adjustment.
* The link:person-accounts[Person Account] record type can be
created from a reference field.
* *Dashboards*. Graphs with summary formulas involved are displayed
correctly on link:home-screen[Home Screen].
...
list) in the related lists link:iphone[on iPhone devices].
...
*Task* and *Event*.
* link:application-editor[Customizable statistics gathering] on
displaying individual screens and elements in CLM presentations.
* link:synchronization[Selective synchronization] in case of user
access rules to object records is changed.
* Now you can sort link:related-lists[the custom and standard
related lists] by lookup fields.
* The *Determine Location* button is not displayed when using
positioning link:actions[on activity start].
* link:clm-presentation-controls[Presentation exit screen
improvements].
* The link:managing-offline-objects[offline-objects] customization
...
* link:sync-logs[Application log records] are more detailed.
* link:person-accounts[Person Accounts] will now display in *Nearby
Accounts* with appropriate settings.
* The improved logic of link:using-calendar[copying activities] in
the calendar.
...
new posts in link:chatter[Chatter].
...
* Updated for iOS 11.
* link:ical-synchronization[Two-way sync with iPad/iPhone internal
...
* Optimized link:synchronization[synchronization] for organizations
with an extended data model and large data volumes.
* Customizable set of link:search[search fields] for each object.
* link:reference-fields#h2__1554731138[Quick record creation in
...
* The link:actions[Determine Geolocation] functionality in the
*Accounts* combo menu now depends on the user’s access rights to the
geolocation field.
* link:debug-console[Application log] search functionality added.
* The *Time* type fields are now supported.
* Multiple currencies functionality added.
* link:home-screen[Customizable Home screen] for iPhone added.
* The link:chatter[Chatter] functionality for iPhone was added.

[[h3_118294372]]
==== CLM-presentations

* JS Bridge methods added:
** link:ctm-disableslidescroll[ctm.isableSlideScroll]: optional
lock for slides scrolling using gestures.
** link:ctm-enableslidescroll[ctm.enableSlideScroll]: slide
scrolling unlock option.
** link:ctm-storefile[ctm.storeFile]: saving a file attachment as
an attachment to a record.
** link:ctm-getfile[ctm.getFile] - getting the contents of an
...
(link:mobile-application-setup[Mobile Application Setup]).
...
link:applications#h3_1236408094[custom scenarios] in the
