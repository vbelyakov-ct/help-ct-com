...
* Fixed adding components to the metadata archive (also when using _xref:ios/admin-guide/metadata-checker/metadata-archive/ability-to-specify-metadata-components-or-the-number-of-entities-in-one-request.adoc[Archive Components]_ custom metadata type).
...
** The ability to use the xref:ios/ct-presenter/js-bridge-api/methods-for-remote-detailing-2-0/ctm-securequery.adoc[cm .secureQuery] method in the CLM presentations to make secure requests to Salesforce data.
