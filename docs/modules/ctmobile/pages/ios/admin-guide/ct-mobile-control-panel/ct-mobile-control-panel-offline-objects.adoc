...
the link:ctm-settings-offline-objects[Offline
Objects] and link:ctm-settings-standard-related-list[Standard
...
the *Selected* list on the link:ct-mobile-control-panel-app-menu[CT
...
If enabled, the link:conflict-manager-control[Conflict Manager]
...
link:other-synchronization-modes#h2__1958232390[sync a single
...
For more information, refer to link:sync-recovery[Sync Recovery].
...
link:managing-offline-objects[Managing Offline Objects].
...
records that match the criteria. Enable link:timeline-view[the
...
For more information, refer to
link:managing-offline-objects[Managing Offline Objects]. Also, get
familiar with the information in the
link:standard-related-lists[Standard Related Lists] article.

To set up custom related lists of an offline object, go
to link:ct-mobile-control-panel-custom-details[CT Mobile Control
