...
Allows the link:ios/conflict-manager-control[Conflict Manager]
...
_true_, then the link:ios/actions#h2_1173923582[Export to File] item is
...
the link:ios/actions#h2_1173923582[Export to File] workflow in
...
an API Name of an link:ios/managing-offline-objects[offline object].
...
Defines if the link:ios/online-records-fetching[Load more records]
...
Allows link:ios/synchronization-launch[Quick Record Synchronization]
...
export after tapping the link:ios/actions#h2_1173923582[Export to
...
Enables link:ios/sync-recovery[Sync Recovery] to deliver records that
...
JSON file in the link:ios/sync-log[Sync Log] object attachments.
