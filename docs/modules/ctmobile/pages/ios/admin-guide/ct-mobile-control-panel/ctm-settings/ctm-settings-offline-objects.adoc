...
Allows the link:conflict-manager-control[Conflict Manager]
...
_true_, then the link:actions#h2_1173923582[Export to File] item is
...
the link:actions#h2_1173923582[Export to File] workflow in
...
an API Name of an link:managing-offline-objects[offline object].
...
Defines if the link:online-records-fetching[Load more records]
...
Allows link:synchronization-launch[Quick Record Synchronization]
...
export after tapping the link:actions#h2_1173923582[Export to
...
Enables link:sync-recovery[Sync Recovery] to deliver records that
...
JSON file in the link:sync-log[Sync Log] object attachments.
