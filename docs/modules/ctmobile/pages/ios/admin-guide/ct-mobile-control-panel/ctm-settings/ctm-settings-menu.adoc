...
Menu]/link:ct-mobile-control-panel-app-menu-new[CT Mobile Control
Panel 2.0: App Menu]. ====
...
a|
Stores an API Name of link:managing-offline-objects[an offline
object], link:mobile-application-modules[module's name], or name of
a menu item, e.g., link:home-screen[Home] and
link:application-settings[Settings], that will be available in
link:app-menu[the main menu].
...
section while creating a new link:list-views#h2__380480215[mobile
...
Defines the way how link:libraries[Library] object must be
downloaded during synchronization, as full content or as a preview.
Interacts with the
link:application-settings#h3_1768799377[Background content loading]
...
link:synchronization[synchronization] with displaying a loader.
** *Background content loading* is disabled: the full content of the
Library object will be downloaded
after link:synchronization[synchronization].
...
|Text(255) a|
Stores an API Name of a field for link:grouping-records[grouping
records] in link:app-menu[the main menu].
...
of link:menu-settings-and-offline-objects[Menu Settings and Offline
