...
* link:reference-fields#h2__1514470758[download additional
record] if the reference field refers to the not downloaded record;
* link:reference-fields#h2__1554731138[search value] or create a
new record to add;
* use link:reference-fields#h2__321327715[lookup filters] to see
records that match required or optional criteria.

[[h2__1514470758]]
=== Online One Record Fetching

Along with link:online-records-fetching[online records fetching], a
...
record, on link:mobile-layouts[the mobile layout] of the linked
record.
...
available link:managing-offline-objects[offline].
...
available offline due to link:related-list-filters[Related List
...
* If a current user launches link:synchronization-launch[one of the
...
* If link:managing-offline-objects#h2_1534686659[a record in the
...
Enter a record in the reference field link:search[using search].
...
using link:mini-layouts[a mini layout].
* Tap image:detailed-list-view-button.png[]
to expand link:list-views#h2_1248088428[the detailed list view].
...
only after link:log-out[the reset database or log out of the
current user].
...
relationship field by link:search[searching] or creating a
...
field by using link:mini-layouts[a mini layout].
* Tap the *Clear* button to clear the value from the reference field. If
a reference field is empty, the *Clear* button is not displayed.
...
rebuild link:metadata-archive[a metadata
archive] using link:metadata-checker[one of the available
