...
device and link:calendar[CT Mobile] calendars.

[NOTE] ==== iCal synchronization settings are saved until the
current user link:log-out[logs out]. link:synchronization[The
...
. link:ical-synchronization[Enable iCal synchronization].
. Select one or both available options:
* link:ical-synchronization[Sync Activities from CT Mobile Calendar
to Device Calendar].

Select a device calendar to synchronize activities from the CT Mobile
calendar.
* link:ical-synchronization[Show Device Calendars Activities in CT
Mobile Calendar].
...
link:compact-layout[a compact layout];
...
link:event-notifications[specify an event notification], and change
