...
device and link:ios/calendar[CT Mobile] calendars.

[NOTE] ==== iCal synchronization settings are saved until the
current user link:ios/log-out[logs out]. link:ios/synchronization[The
...
. link:ios/ical-synchronization[Enable iCal synchronization].
. Select one or both available options:
* link:ios/ical-synchronization[Sync Activities from CT Mobile Calendar
to Device Calendar].

Select a device calendar to synchronize activities from the CT Mobile
calendar.
* link:ios/ical-synchronization[Show Device Calendars Activities in CT
Mobile Calendar].
...
link:ios/compact-layout[a compact layout];
...
link:ios/event-notifications[specify an event notification], and change
