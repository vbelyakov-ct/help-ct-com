...
If enabled, link:demonstration-mode[the demonstration mode] allows
...
device gallery. The new photos in link:actions[the CT Mobile
...
link:ical-synchronization[one-way or both way synchronized] ;with
...
link:event-notifications[a notification of upcoming events]. The
...
If enabled, the current user will receive
link:nearby-accounts-notifications[a notification of nearby
accounts].
...
* Order module application is added to the link:app-menu[App menu].
* The
...
Change link:application-pin-code[the passcode]. The option is
visible when the passcode is enabled
in link:ct-mobile-control-panel-general[CT Mobile Control Panel:
General]/link:ct-mobile-control-panel-general-new#h3_643998525[CT
Mobile Control Panel 2.0: General].

[[h3__1321398704]]
==== Log out

Logging out link:log-out[to reset all application data].
...
CT Mobile allows link:send-application-data-dump[sending a dump]
...
CT Mobile allows link:logging-in[multiple user access]. Changing
...
link:logging-in[To change the current user], select the *User*
...
* *link:log-out[Log out and Erase]*. Tap to log out from the CT
Mobile app.
