...
Control Panel:
General]/link:ct-mobile-control-panel-user-interface-new#h3_1354766135[CT
...
If link:start-finish-functionality[the Start/Finish functionality]
...
[.apiobject]#clm__ActivityDoneAPI__c# field on the link:mobile-application-setup[Mobile Application Setup].#

Tap the icon to view and manage all activities of that day if
link:application-permission-settings[permission to create and
...
In the Applications module, link:applications#h3__1557359819[add
...
presentation mode, use link:gestures-in-clm-presentations[gestures]
and link:clm-presentation-controls[buttons].
...
report, link:ct-mobile-control-panel-tools-new#h3_1003786176[refresh
...
to link:managing-offline-objects[CT Mobile Control Panel: Managing
Offline
...
link:mobile-layouts-dashboards[the mobile layout], only offline
...
link:sync-logs[Sync Logs reports] or
link:clm-applicationstats[CLM-presentation statistics], the message
«_No data to display_» will be shown on the Dashboard widget. A mobile
user can view these dashboards online in link:dashboards[the
...
about the data relevancy and if link:synchronization[the
...
information, go to link:synchronization-launch[Synchronization
Launch].
