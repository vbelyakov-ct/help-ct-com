...
corner of the link:home-screen[Home Screen] or long tap the *Sync*
...
The changes in link:clm-user[the audit fields] of the current
...
link:sync-recovery[the Sync Recovery functionality].
...
go to link:metadata-checker[Metadata Checker].
...
link:metadata-archive[metadata components] and update the current
database.
Metadata components are downloaded from ZIP archive(s) or directly using
administrator credentials depending
on link:ct-mobile-managed-package-update-to-v-3-54[the CT Mobile
...
that case, link:validation[validation rules], offline reports, and
...
link:mobile-application-modules[modules], are downloaded if the
...
matches the conditions set in link:related-list-filters[related
list filters].

ifdef::ios[]

If configured, the records of the specified objects mentioned in the
link:ct-mobile-replication[CT Mobile Replication] setting are
updated.

link:clm-user[The audit fields] of the current user record are
updated.
