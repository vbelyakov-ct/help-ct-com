...
corner of the xref:home-screen[Home Screen] or long tap the *Sync*
...
go to xref:metadata-checker[Metadata Checker].
...
xref:metadata-archive[metadata components] and update the current
database.
...
xref:mobile-application-modules[modules], are downloaded if the
...
xref:ct-mobile-replication[CT Mobile Replication] setting are
updated.
