...
synchronization statistics, refer to link:ios/sync-logs[Sync Logs].

[[h3__21591833]]
==== First Full Synchronization

The entire database of the CT Mobile app,
link:ios/metadata-archive[metadata components], and app settings will
be downloaded from Salesforce to the mobile device.

This synchronization should be used after link:ios/logging-in[the
...
For more information, go to link:ios/full-synchronization[First Full
...
Enable link:ios/ct-mobile-replication[CT Mobile Replication] if
territory coordinates have been altered to download all actual data on
the mobile device.

Set up link:ios/sync-recovery[Sync Recovery] to have the ability to
upload records with the errors to Salesforce.

For more information, go to link:ios/fast-synchronization[Fast and
...
the link:ios/mobile-application-setup[Mobile Application Setup] custom
...
* link:ios/other-synchronization-modes#h2__1958232390[Quick Record
Synchronization]
Used to update a single record and its child records by using the
pull-to-refresh gesture on the record's layout. Requires
permission (refer to
link:ios/ct-mobile-control-panel-offline-objects#h3_202390671[CT Mobile
Control Panel: Offline
objects]/link:ios/ct-mobile-control-panel-offline-objects-new#h4_202390671[CT
Mobile Control Panel 2.0: Offline Objects]).
You can update link:ios/chatter[the Chatter feed] the same way but this
functionality does not depend on permission to quick record
synchronization.
* link:ios/other-synchronization-modes#h2_740581689[Workflow
Synchronization]
This mode of synchronization is run when link:ios/ct-mobile-workflow[CT
Mobile Workflow] conditions are triggered.
* link:ios/other-synchronization-modes#h2_233027861[CT Presenter
Synchronization]
This mode of synchronization is triggered when the
link:ios/ctm-sync[sync()] method of JS Bridge is requested.

For more information, go to link:ios/other-synchronization-modes[Other
Synchronization Modes].
...
. Open the link:ios/home-screen[Home screen].
...
link:ios/metadata-archive[the metadata archive] must be collected and
...
** *Mobile Version*
** *Operation System*
* The link:ios/sync-logs[Sync Log] records store the information of
...
link:ios/logging-in[the authorization] of the current user.

The first full synchronization is intended to download the entire
database of the CT Mobile app, link:ios/metadata-archive[metadata
components],
link:ios/user-permissions-required-to-synchronize-objects-to-offline-work-with-functionalities[required
...
. Records of link:ios/custom-settings[the custom settings and custom
...
the link:ios/ct-mobile-replication[CT Mobile Replication] setting are
updated.
...
to link:ios/android-2-0-sync#h3__479754125[the Errors screen] and tap a
