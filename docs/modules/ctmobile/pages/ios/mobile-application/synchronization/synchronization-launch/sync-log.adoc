...
link:ios/synchronization[synchronization] depending on the value in the
*Debug Level* option on the
link:ios/ct-mobile-control-panel-general#h3_1226274811[CT Mobile
Control Panel:
General]/link:ios/ct-mobile-control-panel-general-new#h3_1226274811[CT
...
user link:ios/send-application-data-dump[sends an application data
dump], the field contains [.apiobject]#Part n from m#,
where *n* is the number of an attachment and *m* is the total number of
attachments. ifdef::ios[]If the link:ios/sync-recovery[Sync
Recovery] functionality is enabled with the *Proxy Object* parameter
specified
on link:ios/ct-mobile-control-panel-offline-objects#h3_37043680[CT
Mobile Control Panel: Offline
Objects]/link:ios/ct-mobile-control-panel-offline-objects-new#h4_37043680[CT
...
If link:ios/ct-mobile-control-panel-general#h3__1808523151[the tracking
interval] (refer to
...
file in the Attachments section if the link:ios/sync-recovery[Sync
