...
the link:ct-mobile-control-panel[CT Mobile Control
...
For more information, read link:full-synchronization[the Full
Synchronization article].



Go back to link:synchronization-f-a-q#F.A.Q.list[the F.A.Q. list].
...
link:publishing-clm-presentations[republishing]. The CT Mobile app
...
link:clm-application[on the CLM presentation record], the CT mobile
...
could be managed on link:errors-screen[the Errors screen].

The CT Mobile app is not locked during the fast synchronization.

ifndef::win,andr[]

The sync process may include link:ct-mobile-replication[CT Mobile
Replication];

The sync process may include the link:sync-recovery[Sync Recovery]
functionality.



For more information, read link:fast-synchronization[the Fast
Synchronization article].
...
link:clm-user[current user's record] is earlier than the date of
the last metadata change in the
[.apiobject]#clm__MetadataLastModifiedDate__c# field of
the link:mobile-application-setup[Mobile Application Setup] record
...
* download link:metadata-archive[metadata] changes in ZIP
...
a change on link:ct-mobile-control-panel[CT Mobile Control
Panel]/link:ct-mobile-control-panel-new[CT Mobile Control Panel
2.0].
...
Errors could be managed on link:errors-screen[the Errors screen];
...
* link:metadata-checker#h2__682427009[Manually create the metadata
archive].
* link:metadata-checker#h2__1244689107[Enable Metadata Checker] to
refresh metadata.
...
example, link:remote-detailing-launch-the-ct-mobile-app[to download
...
For more information, read link:sync-recovery[the Sync Recovery
...
case, link:ct-mobile-replication[create a CT Mobile Replication
...
link:ct-mobile-replication-use-cases-and-steps[here].
...
on link:home-screen#h2__396225247[the status indicator] at the
...
information, read link:sync-log[the Sync Log article].
...
The link:sync-log[Sync Log] object is intended to store logs for
...
link:send-application-data-dump#SendApplicationDataDump-SpecifyDatatoSend[Sync
...
For more information, read link:sync-logs[the Sync Logs article].
...
link:attachments-and-files[attachments] can be downloaded one at a
time.
...
link:metadata-archive#h2_1854953360[here]. Also, the metadata
...
We recommend enabling link:metadata-checker[a metadata checker] and
link:metadata-checker#h3_1645525423[setting a schedule] for a
...
may incorrectly work, for example, link:validation[offline
validation rules].

. Check link:ct-mobile-control-panel-tools#h3_2011978[the cloud
token] (refer to link:ct-mobile-control-panel-tools#h3_2011978[CT
Mobile Control Panel:
Tools]/link:ct-mobile-control-panel-tools-new#h2_2011978[CT Mobile
Control Panel 2.0: Tools]) is valid.
. Create a ZIP archive or refresh metadata in the ZIP archive. We
recommend enabling link:metadata-checker[a metadata checker]. Also,
it is possible link:metadata-checker#h3_1645525423[to set a
schedule] for a metadata checker or
link:metadata-checker#h2__1657467916[manually create/update] the
metadata archive.
...
created or updated. We recommend enabling link:metadata-checker[a
metadata checker]. Also, it is possible
link:metadata-checker#h3_1645525423[to set a schedule] for a
metadata checker or link:metadata-checker#h2__1657467916[manually
create/update] the metadata archive.
* Also, check the link:application-language[Application Language]
article.
...
link:metadata-archive[a metadata archive]. We recommend enabling
link:metadata-checker[a metadata checker]. Also, it is possible
link:metadata-checker#h3_1645525423[to set a schedule] for a
metadata checker or link:metadata-checker#h2__1657467916[manually
create/update] the metadata archive.
...
Go to the link:send-application-data-dump[Send Application Data
Dump] article.
...
link:technical-requirements-for-devices-and-network#h2_1498128754[the
requirements for the connection].
* amount of data. To avoid the synchronization of the large data,
link:managing-offline-objects[customize the criteria] to download
desired records of offline objects.

* amount of modified components (objects, fields, layouts) or changes
provided on the link:ct-mobile-control-panel[CT Mobile Control
Panel]/link:ct-mobile-control-panel-new[CT Mobile Control Panel
...
link:managing-offline-objects[SOQL query] in the error message.
...
inserted into link:reference-fields[the reference field] when the
...
* Enable the link:application-settings#h3_1768799377[Background
...
* Disable the link:ctm-settings-offline-objects[Files Download
Enabled] option for offline objects. The files attached to offline
objects will be downloaded as previews.
* Set up the link:ctm-settings-offline-objects[Files Filter] option
...
Enable logs on the
link:ct-mobile-control-panel-general#h3__272409891[CT Mobile
Control Panel:
General]/link:ct-mobile-control-panel-general-new#h2__1687169837[CT
Mobile Control Panel 2.0: General] tab to collect statistics in the
[.object]#Sync Log# object. The link:sync-log[Sync Log]
