...
link:mobile-layouts-applications[mobile layout] and manage CLM
presentations by link:clm-presentation-controls[the control
buttons], link:gestures-in-clm-presentations[gestures], or
link:js-bridge-api[JS methods and links] on the slides. For each
...
link:full-synchronization[full synchronization], as it is stored
locally in the mobile app. For link:clm-customscenario[custom
...
editing link:custom-scenario-editor[custom CLM presentation
scenarios] in the CT Mobile app.

The *Slide reordering enabled* checkbox must be selected in the
link:clm-application[Application record type].

Depending on[.object]#CLM presentation# settings on the
link:clm-slide[Slide] object, it provides the possibility to change
...
configured as link:clm-slide[Required] and cannot be excluded from
the custom scenario:
image:custom-scenario-required-slides.png[]

The scenario is created.
link:synchronization-launch#h3__116633872[Launch fast
...
available link:synchronization[at the first synchronization] or
...
. Tap *Send Selected*.
. Select link:email-templates[an email template] if available.
