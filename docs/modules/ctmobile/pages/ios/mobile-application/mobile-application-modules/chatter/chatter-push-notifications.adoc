...
xref:ios/admin-guide/mobile-layouts/mobile-layouts-chatter-feed.adoc[Chatter feed] is a widget to display messages on the *Chatter* module screen or in the *Chatter* tab of the selected record.
...
. In xref:ios/admin-guide/ct-mobile-control-panel/custom-settings/mobile-application-setup.adoc[Mobile Application Setup], set the value for the xref:ios/admin-guide/push-identifier.adoc[PUSH Identifier] field.
+
NOTE: For the CT Mobile app installed from the App Store, the push identifier is [.apiobject]#ctmobile#.
...
. Please make sure that the *Device Token* field for the current record of the [.object]#User# object is filled with a value.
+
...
+
NOTE: This sync process is not affected by the *Quick Record Sync* setting for the object on the *CT Mobile Control Panel: Offline Objects* tab.
+
image::62556898.png[]
