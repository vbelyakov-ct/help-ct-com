...
. Please set up link:remote-site-settings[the required remote
sites].
. Assign link:managing-ct-mobile-licenses[CT Mobile license] to
users.
. Specify link:application-permission-settings[permissions to
users]:
.. Check that the user's profile has permission to read or edit
necessary objects and fields.
.. link:application-permission-settings#h3_2115044027[Assign
required permission sets] to users according to their roles.
. ​Ensure that all required objects
link:managing-offline-objects#h2_1551357854[are available offline]
and link:managing-offline-objects#h2_1814060569[use SOQL filters]
to reduce the synchronization time and the amount of data to download.
. link:installing-the-ct-mobile-app[Download the CT Mobile app].
. link:logging-in[Log in] to the CT Mobile app using your
Salesforce credentials.
. Perform link:synchronization-launch[the first full
synchronization].
...
link:clickjack-protection-settings[here].
