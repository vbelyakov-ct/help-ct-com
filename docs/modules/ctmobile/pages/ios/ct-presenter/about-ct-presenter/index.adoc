...
If you want to use link:migration-to-remote-detailing-2-0[Remote
...
. Go to link:ct-mobile-control-panel-tools#h3_2011978[CT Mobile
Control Panel:
...
. Assign link:application-permission-settings#h3_2115044027[the
preconfigured permission set] to a user according to their role.
. Check link:user-permissions[user permissions] to CLM objects.
. Get familiar with link:requirements-and-media-file-formats[the
...
CT Presenter provides the link:application-editor[Application
Editor]:
...
* Add link:creating-clm-presentation-from-powerpoint[PowerPoint],
link:creating-clm-presentation-from-pdf[PDF], or
link:creating-clm-presentation-from-jpeg-jpg[JPEG/JPG] files to
convert into a CLM presentation.



CT Presenter provides the link:plain-application-editor[Plain
Application Editor]:

* Create and edit CLM presentations
link:creating-plain-clm-presentation[using the web interface].
* link:creating-plain-clm-presentation-using-templates[Create your
templates], and slides with images, texts, videos, etc.
* link:creating-plain-clm-presentation-from-powerpoint[Import
PowerPoint] into an existing template and convert it to a CLM
presentation.



Use link:custom-scenario-editor[the custom scenario editor] to
create, edit and delete custom scenarios.

ifdef::ios,win[]

In the CT Mobile app, you can
specify link:clm-customscenario[custom scenarios] with the help of
the link:applications#h3_1236408094[Mobile Scenario Editor] in the
Application module.
...
* link:publishing-clm-presentations#h3_1098755975[Renew slides] of
...
link:publishing-clm-presentations#h2_1149854286[on the Applications
tab].
* Publish a CLM presentation by using
link:publishing-clm-presentations#h2_104022594[the Application
Editor].
...
to link:mobile-application-setup#CLMDisplayMode[specify the CLM
...
* The link:clm-application[CLM presentation] should be active and
available offline.
...
* link:applications[The Application module] allows reviewing CLM
presentations uploaded from Salesforce.
* You can configure link:mobile-layouts-applications[the mobile
...
* Use pre-configured link:gestures-in-clm-presentations[gestures].
* Double-tap the left or right side of the screen to view the
link:clm-presentation-controls[control buttons].
* Double-tap the bottom of the screen to display the selected scenario
and slide in link:clm-navigation-in-clm-presentations[a branch] for
rapid switching between the slides.
* link:js-bridge-api[JS Bridge methods] are available from within
the CLM presentation if specified on the slide markup.
* During the CLM presentation, specific user actions are captured and
collected in the link:clm-applicationstats[Application Stats]
...
link:remote-detailing-f-a-q[Remote Detailing: F.A.Q]. ====
...
link:remote-detailing-launch-the-ct-mobile-app[for Remote Detailing
on the Activity record in the CT Mobile app].
* The presenter can launch a CLM presentation
link:remote-detailing-launch-salesforce-side[for Remote Detailing
on the Activity record or the Application record in Salesforce].
* The presenter can launch a CLM presentation
link:remote-detailing-launch-microsoft-teams[via Microsoft Teams].
...
link:remote-detailing-f-a-q[]JS methods and links are available
...
according to link:clm-settings[the Session timeout (in seconds)
field].
