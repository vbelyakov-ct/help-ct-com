...
may be used link:changing-slides[in the navigation] for switching
...
the link:clm-applicationstats[Application Stats] object.
...
link:creating-clm-presentation-from-powerpoint[PowerPoint],
link:creating-clm-presentation-from-pdf[PDF], or
link:creating-clm-presentation-from-jpeg-jpg[ZIP archives with the
JPEG/JPG files] on the link:application-editor[Application Editor]
tab.
* *Plain Application*

In the built-in link:plain-application-editor[Plain Application
Editor], CLM presentations can be created based on
link:clm-template[templates],
link:clm-placeholder[placeholders], link:clm-widget[widgets],
...
link:clm-customscenario[custom scenarios].

|Slide Zoom Enabled |[.apiobject]#clm__IsZoomEnabled__c#
|Checkbox |If enabled, zoom in and out slides if
link:mobile-application-setup#h2_993073976[the display mode is
*wk*].
...
* link:the-remote-detailing-functionality[For Remote Detailing]
