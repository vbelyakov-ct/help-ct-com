...
device and link:calendar.html[CT Mobile] calendars.

[NOTE] ==== iCal synchronization settings are saved until the
current user link:log-out.html[logs out]. link:synchronization.html[The
...
. link:ical-synchronization.html[Enable iCal synchronization].
. Select one or both available options:
* link:ical-synchronization.html[Sync Activities from CT Mobile Calendar
to Device Calendar].

Select a device calendar to synchronize activities from the CT Mobile
calendar.
* link:ical-synchronization.html[Show Device Calendars Activities in CT
Mobile Calendar].
...
link:compact-layout.html[a compact layout];
...
link:event-notifications.html[specify an event notification], and change
