...
. In the xref:ctmobile:ios/mobile-application/ui/actions.adoc[Actions] menu, tap the *Sign Document* button.
...
. Tap xref:admin-guide/configuring-the-ct-sign-package/index.adoc#h2_236049169[the object to sign] in the left menu.
. Select a record in the left record list.
. In the xref:ctmobile:ios/mobile-application/ui/actions.adoc[Actions] menu, tap *Sign Document*.
+
...
. (Optional) select an xref:ctmobile:ios/mobile-application/email-templates.adoc[Email template], if you want to use it for sharing the document by email. Otherwise, tap *Skip*.
...
If any errors occur on the records described above, the sync process will include xref:ctmobile:ios/mobile-application/synchronization/sync-recovery.adoc[the Sync Recovery functionality].

. If an error occurs while sending the _Sign Document_ record (or any of the records that it refers to via reference fields), then the _Sign Document_ record will be sent with the help of the xref:ctmobile:ios/mobile-application/synchronization/sync-recovery.adoc#h3_459631233[Direct access] option.
...
. If records in steps 1 or 3 could not be sent via the *Direct access* option, the CT Mobile app tries to send them using the xref:ctmobile:ios/mobile-application/synchronization/sync-recovery.adoc#h3_356910769[Proxy object] option. If unsuccessful, a corresponding error will be listed on the xref:ctmobile:ios/mobile-application/synchronization/errors-screen.adoc[Errors] screen.
