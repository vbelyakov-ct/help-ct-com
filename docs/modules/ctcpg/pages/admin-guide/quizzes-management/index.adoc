...
. link:override-basic-action-for-quiz[Override Basic Action for
...
the Quizzes tab].
. Define the __Quiz __purpose and link:create-a-new-quiz[create a
new Quiz] with the required options:
* link:specify-questions-for-quiz[create questions] for the quiz;
* specify the order of the questions;
* activate the _Quiz_ record.
. link:assign-the-quiz-partaker[Assign representatives] to pass the
quiz.
