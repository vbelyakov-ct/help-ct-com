...
xref:calculation-types[Calculation Type] in multiple steps of one
xref:pricing-procedure-v-2-steps[pricing procedure].

The xref:calculated-discount-field-reference[Calculation Discount]
record will store the discount based on the last step of the pricing
procedure. It is recommended to use xref:step-conditions[conditions
...
of the xref:freebie-condition-field-reference[Freebie Condition]
object and the *Condition Details* and *Advanced Criteria* fields of the
xref:condition-field-reference[Condition] object if
