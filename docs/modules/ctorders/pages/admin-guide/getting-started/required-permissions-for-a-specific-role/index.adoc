...
WARNING: To work with xref:admin-guide/managing-ct-orders/web-service/index.adoc[Web Service], the integration user must have permission to carry out discount calculations using the Web Service. For more information, refer to
xref:attachment$CT-Orders-User-Permissions-to-Access-Web-Service.pdf[Integration User Permissions].
