...
When all settings are done, let's create a new order to apply the configured discounts and fees:
...
. On the left side, expand the *Catalogs* or *Promotion* in the corresponding section, select a catalog, promotion, or product, and click *Add*.
...
.. If you do not reach the required total price for the whole order cart, the total fee per order will be applied.
+
