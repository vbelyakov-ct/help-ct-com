...
can link:connecting-to-web-service-and-price-calculation-7-0[choose
...
required
permissions link:../Storage/project-order-module/PDF/CT-Orders-User-Permissions-to-Access-Web-Service.pdf[here]. 
* Tap
the image:WS-Calculation-in-CT-Mobile.png[] button
in the link:offline-order#h4__1635896381[order cart] to get the
...
* link:calculation-type-applyconditiontype-c-field-specification[Apply
...
* link:links[Links]: Support the *Reduce* dependency action to
calculate discounts for Freebies.
* Edit fields with the *Currency*, *Number,* *Percent*, or *Text* types
within the link:delivery-management#h2__1374863314[Delivery
Summary] pop-up.
* The UX/UI improvement for[.object]#Quotas#:
** Display the current quota status in the
link:offline-order#h4__1635896381[order cart] interface.
** For better visibility, the link:quotas#h2_275625523[quota info
...
** link:offline-order#h4__589780300[In the catalog and promotion
...
* Downloading link:limit-rules[Limit Rules] during the sync
process.
* link:limit-rules[Applying limit rules] to orders or deliveries
...
* link:product-availability[Dynamic control] over the availability
of products, bundles, and freebies for order and deliveries on certain
dates.
* Support for link:product-data-model[new product fields] to
implement the product availability logic.
* During the sync process, the
link:product-availability-field-reference[Product Availability]
...
* link:product-availability#h2_813222365[Tap the Info icon] next to
...
* Dynamic control of link:quota-field-reference[a limited number of
...
* link:quotas#h2_275625523[Tap the Info icon] next to the product
...
link:catalogs-field-reference[Disable Mass Action] checkbox is
inactive.
...
* On the link:offline-order#h4__1635896381[Order Cart] screen, the
...
link:offline-order#h3_1847490047[the Edit Cart screen]:
...
link:managing-bundles[A bundle] is a new type of product that
...
link:product-data-model[units in a package] on the *Product* level.

[[h3__230916525]]
==== Other Improvements

* Freebies management window with the dedicated button.
* link:5-2-setting-up-the-delivery-summary[Delivery summary] window
