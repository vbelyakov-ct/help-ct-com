...
|Display Type |The display type to define the format of the Dialog Section extension. |*Hierarchy View* in the example below.
|===
...
|Extension Object |API name of the object to display in *the hierarchy view*.  | |

|Lookup from Extension to Current Object |The API name of the field with the *Lookup* type is to associate the current object and the *Extension Object*. | |
|===
...
|Required Attribute |If selected, the field (a column) is required. |Comma-separated, without blank spaces, in the same order as in the *Fields Layout* field.
