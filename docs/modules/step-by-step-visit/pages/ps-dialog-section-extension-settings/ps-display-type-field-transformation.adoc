...
|Display Type |The display type to define the format of the Dialog Section extension. |*Field Transformation* in the example below.
...
|Field Layout |The API names of the fields (columns) that must be displayed. |
|===
...
|Pass To PDF |If selected, the Barcode has to have proceeded to PDF. a|NOTE: Not in use.
...
|Image Transformation Field |When a field must be transformed into an image. |This field is used when *List View Screen* is selected as the *Display Type*.

|PDF Transformation Field |When a field must be transformed to PDF on the layout. |Very specific case.
|===
